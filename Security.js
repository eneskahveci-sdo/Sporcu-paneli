...
// Sayfanın üstünde konsol loglarını gösterir.
// Kapatmak için: window._debugPanel = false; yazıp sayfayı yenileyin.
...
(function initDebugPanel() {
    ...
    // console.log/warn/error/info'yu hook'la
    ...
    // Yakalanmamış hataları da göster
...
function getAuthClient() {
    // Önce window.getSupabase dene (script.js'den)
    if (window.getSupabase) {
        try { return window.getSupabase(); } catch(e) {}
    }
    // Fallback: AppState.sb
...
// Security.js v4.0 — script.js'deki doNormalLogin'i kesin olarak ezer.
// DOMContentLoaded sonrasında da override yapar (yükleme sırası sorunu fix).
            ...
            if (role === 'coach') {
                ...
                if (coachRow) {
                    ...
                    if (typeof window.updateBranchUI === 'function') {
                        try {
                            window.updateBranchUI();
                            console.log('✅ updateBranchUI tamamlandı');
                        } catch(ubErr) {
                            console.error('🔴 updateBranchUI hatası:', ubErr);
                        }
                    }
...
// DOMContentLoaded'da tekrar override et — script.js sonradan yüklense bile
// Security.js'in versiyonu geçerli olsun
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(function() {
        window.doNormalLogin = function(role) {
            return _securityDoNormalLogin(role)();
