...
window.doNormalLogin = async function(role) {
    ...
    try {
        ...
        // ADIM 2: İlk kez giriş veya şifre değişmiş → migrasyon
        if (authError && (authError.message.includes('Invalid login') || authError.status === 400)) {
            ...
            if (isValid) {
                // Önce mevcut auth kaydını sil (şifre uyuşmazlığı olabilir)
                await sb.rpc('delete_auth_user_by_email', { p_email: email }).catch(() => {});
