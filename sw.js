...
const STATIC_CACHE = 'dragos-static-v21';
const API_CACHE = 'dragos-api-v21';
...
// ── Push bildirimi alındığında göster ─────────────────────────────────────────
self.addEventListener('push', function (event) {
    var data = {};
    try { data = event.data ? event.data.json() : {}; } catch (e) { data = { title: 'Bildirim', body: event.data ? event.data.text() : '' }; }
    event.waitUntil(
