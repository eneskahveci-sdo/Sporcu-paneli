// ═══════════════════════════════════════════════════════════
// HATA DÜZELTMELERİ V7 — script.js'den SONRA yüklenir
//
...
// - Ön Kayıt sayfasına "Düzenle" butonu eklendi
// - Mobil responsive iyileştirmeleri
// ═══════════════════════════════════════════════════════════

console.log('script-fixes.js V7 yukleniyor...');
...
function _openLegalOverlay(title, body) {
    ...
    box.style.cssText = 'background:var(--bg,#0f1623);border:1px solid var(--border,#1e3352);border-radius:16px;width:100%;max-width:600px;max-height:90vh;max-height:90dvh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,.5);overflow:hidden';
    ...
    bodyEl.style.cssText = 'padding:20px;overflow-y:auto;flex:1;-webkit-overflow-scrolling:touch;overscroll-behavior:contain';
    bodyEl.innerHTML = body;

    // Footer
    var foot = document.createElement('div');
    foot.style.cssText = 'padding:16px 20px;border-top:1px solid var(--border,#1e3352);display:flex;justify-content:flex-end;background:var(--bg2,#0f1623);border-radius:0 0 16px 16px;flex-shrink:0;padding-bottom:max(env(safe-area-inset-bottom,16px),16px)';
...
console.log('KVKK/Kullanim showLegal tanimlandi (V7 - bagimsiz overlay)');
...
// ────────────────────────────────────────────────────────
// 5b) ÖN KAYIT SAYFASI RENDER — ayrı sayfa + Düzenle butonu
// ────────────────────────────────────────────────────────
function __renderOnKayit() {
    ...
    if (onKayitlar.length === 0) {
        ...
        onKayitlar.forEach(function(ok) {
            ...
            var btns = '';
            if (ok.status === 'new') {
                btns += '<button class="btn btn-xs bp" onclick="convertOnKayit(\'' + ok.id + '\')" style="margin-bottom:4px">&#x2705; Onayla</button> ';
                btns += '<button class="btn btn-xs bs" onclick="editOnKayit(\'' + ok.id + '\')" style="margin-bottom:4px">&#x270F; D\u00FCzenle</button> ';
            }
        ...
        inner = '<div class="tw" style="overflow-x:auto;-webkit-overflow-scrolling:touch"><table><thead><tr><th>Tarih</th><th>Ad Soyad</th><th>TC</th><th>Do\u011Fum</th><th>S\u0131n\u0131f Talebi</th><th>Veli / Telefon</th><th>Durum</th><th>\u0130\u015Flemler</th></tr></thead><tbody>' + rows + '</tbody></table></div>';
    }
...
// ────────────────────────────────────────────────────────
// 5c) editOnKayit — Ön kayıt düzenleme modal'ı (YENİ V7)
// ────────────────────────────────────────────────────────
window.editOnKayit = function(id) {
    ...
    var clsOpts = classes.map(function(c) {
        var sel = (ok.clsId === c.id || ok.className === c.name) ? ' selected' : '';
        return '<option value="' + FormatUtils.escape(c.id) + '" data-name="' + FormatUtils.escape(c.name) + '"' + sel + '>' + FormatUtils.escape(c.name) + '</option>';
    }).join('');

    var h = '<div class="g21 mb2">'
        + '<div class="fgr"><label>\u00D6\u011Frenci Ad\u0131 *</label><input id="eok-fn" value="' + FormatUtils.escape(ok.fn || '') + '"/></div>'
        + '<div class="fgr"><label>\u00D6\u011Frenci Soyad\u0131 *</label><input id="eok-ln" value="' + FormatUtils.escape(ok.ln || '') + '"/></div>'
        + '</div>'
        + '<div class="g21 mb2">'
        + '<div class="fgr"><label>Do\u011Fum Tarihi *</label><input id="eok-bd" type="date" value="' + FormatUtils.escape(ok.bd || '') + '"/></div>'
        + '<div class="fgr"><label>TC Kimlik No</label><input id="eok-tc" type="text" inputmode="numeric" maxlength="11" value="' + FormatUtils.escape(ok.tc || '') + '"/></div>'
        + '</div>'
        + '<div class="fgr mb2"><label>S\u0131n\u0131f Talebi *</label><select id="eok-cls"><option value="">S\u0131n\u0131f Se\u00E7iniz</option>' + clsOpts + '</select></div>'
        + '<div class="dv"></div>'
        + '<div class="tw6 tsm mb2">Veli Bilgileri</div>'
        + '<div class="fgr mb2"><label>Veli Ad\u0131 Soyad\u0131 *</label><input id="eok-pn" value="' + FormatUtils.escape(ok.parentName || '') + '"/></div>'
        + '<div class="fgr mb2"><label>Veli Telefon *</label><input id="eok-pph" type="tel" value="' + FormatUtils.escape(ok.parentPhone || '') + '"/></div>';

    modal('\u270F\uFE0F \u00D6n Kay\u0131t D\u00FCzenle', h, [
        ...
        { lbl: 'Kaydet', cls: 'bp', fn: async function() {
            ...
            if (clsEl && clsEl.selectedIndex >= 0) {
                var opt = clsEl.options[clsEl.selectedIndex];
                clsName = (opt && opt.dataset && opt.dataset.name) || (opt ? opt.textContent : '') || '';
            }
            ...
            if (idx >= 0) {
                AppState.data.onKayitlar[idx].fn = fn;
                AppState.data.onKayitlar[idx].ln = ln;
                AppState.data.onKayitlar[idx].studentName = fn + ' ' + ln;
                AppState.data.onKayitlar[idx].bd = bd;
                AppState.data.onKayitlar[idx].tc = tc;
                AppState.data.onKayitlar[idx].clsId = clsId;
                AppState.data.onKayitlar[idx].className = clsName;
                AppState.data.onKayitlar[idx].parentName = pn;
                AppState.data.onKayitlar[idx].parentPhone = pph;
            }
            ...
            try {
                var sb = getSupabase();
                if (sb) {
                    var res = await sb.from('on_kayitlar').update({
                        student_name: fn + ' ' + ln,
                        fn: fn,
                        ln: ln,
                        bd: bd || null,
                    ...
                    if (res.error) {
                        console.warn('editOnKayit DB hata:', res.error);
                        toast('Veritaban\u0131 g\u00FCncelleme hatas\u0131: ' + (res.error.message || ''), 'e');
                    } else {
                        toast('\u2705 \u00D6n kay\u0131t ba\u015Far\u0131yla g\u00FCncellendi!', 'g');
                    }
                } else {
                    toast('\u2705 Yerel olarak g\u00FCncellendi (DB ba\u011Flant\u0131s\u0131 yok)', 'g');
                }
            } catch(e) {
                console.error('editOnKayit exception:', e);
                toast('Hata: ' + (e.message || 'Bilinmeyen'), 'e');
            }

            closeModal();
            // Sayfayı yenile
            if (AppState.ui && AppState.ui.curPage === 'onkayit') go('onkayit');
            else if (AppState.ui && AppState.ui.curPage === 'athletes') go('athletes');
...
console.log('\u2705 Script-fixes V7 yuklendi (KVKK overlay + iOS fix + editOnKayit)');

// ────────────────────────────────────────────────────────
// 11) Ayarlar sayfası ön kayıt bölümüne "Düzenle" butonu ekle
//     (pgSettings script.js'de render edildikten sonra DOM'u güncelle)
// ────────────────────────────────────────────────────────
(function() {
    // Her go() çağrısından sonra settings sayfasını kontrol et
    var _prevGo = window.go;
    var _patchTimer = null;

    function patchSettingsOnKayit() {
        // Settings sayfasındaki "Sporcuya Dönüştür" butonlarının yanına "Düzenle" ekle
        var cards = document.querySelectorAll('.onkayit-card');
        cards.forEach(function(card) {
            // Zaten düzenle butonu varsa atla
            if (card.querySelector('[data-edit-ok]')) return;

            var convertBtn = card.querySelector('.btn.bsu');
            if (!convertBtn) return;

            // convertOnKayit('xxx') → id'yi çıkar
            var onclickStr = convertBtn.getAttribute('onclick') || '';
            var match = onclickStr.match(/convertOnKayit\('([^']+)'\)/);
            if (!match) return;
            var okId = match[1];
            ...
            editBtn.style.cssText = 'margin-left:4px';
            editBtn.onclick = function() { window.editOnKayit(okId); };

            // convertBtn'den sonra ekle
            convertBtn.parentNode.insertBefore(editBtn, convertBtn.nextSibling);
        });
    }

    // go() override — settings sayfası açıldığında DOM'u güncelle
    window.go = function(page, params) {
        var result = _prevGo.call(window, page, params);

        // Settings sayfası render'ı async (setTimeout(100ms)) olduğundan biraz bekle
        if (page === 'settings') {
            clearTimeout(_patchTimer);
            _patchTimer = setTimeout(patchSettingsOnKayit, 250);
        }
...
console.log('\u2705 Script-fixes V7 settings patch yuklendi');
