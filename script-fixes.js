// ═══════════════════════════════════════════════════════════
// HATA DÜZELTMELERİ V8 — script.js'den SONRA yüklenir
//
// V8 DEĞİŞİKLİKLER:
// - KRİTİK: spTab() data-tab attribute ile eşleştirme (tab kaybolma fix)
// - Gelişmiş pgAccounting: Aylık grafik, kasa/banka, gider kategorileri
// - Gider kategorileri sistemi
// - Makbuz/Dekont oluşturma (jsPDF)
// - Kasa ve Banka takibi
// - WhatsApp Business API entegrasyonu
// - Önceki V7 özellikleri korundu
// ═══════════════════════════════════════════════════════════

console.log('script-fixes.js V8 yukleniyor...');
...
window.generateReceipt = function(paymentId) {
    ...
    try {
        ...
        addRow('Sporcu:', p.an || '-');
        addRow('Aciklama:', p.serviceName || p.ds || 'Aidat');
        addRow('Tutar:', FormatUtils.currency(p.amt));
        addRow('Odeme Yontemi:', statusLabel(p.payMethod || '-'));
        addRow('Durum:', p.st === 'completed' ? 'Odendi' : statusLabel(p.st));
        if (p.slipCode) addRow('Slip Kodu:', p.slipCode);
...
// ────────────────────────────────────────────────────────
if (!AppState.data.cashTransfers) AppState.data.cashTransfers = [];
...
window.sendWhatsAppMessage = async function(phone, message) {
    ...
    var cleanPhone = phone.replace(/[\s\-\(\)]/g, '').replace(/^0/, '90').replace(/^\+/, '');
...
console.log('✅ Script-fixes V8 yüklendi — spTab fix + finans grafikleri + makbuz + kasa + WhatsApp');
/* =========================================================================
   DRAGOS AKADEMİ V8 - YENİ ÖZELLİKLER VE MENÜ DÜZELTMESİ
   Bu kod mevcut script-fixes.js dosyasının en altına eklenmelidir.
   ========================================================================= */
...
(function initV8Patch() {
    ...
    const v8Styles = `
        <style>
            ...
            @media print {
                ...
                #print-receipt-container { display: block !important; position: absolute; left: 0; top: 0; width: 100%; padding: 20px; font-family: sans-serif; background: #fff; z-index: 9999; }
    ...
    // 3. CHART.JS VE MAKBUZ DOM EKLEMESİ
    if (!window.Chart) {
        ...
        const receiptHTML = `
            <div id="print-receipt-container" style="display: none;">
                ...
                <div class="receipt-body">
                    ...
                    <p><strong>Yöntem:</strong> <span id="receipt-method"></span></p>
    ...
    // 4. DB MAPPERS VE SP-TAB (MENÜ) DÜZELTMESİ
    setTimeout(() => {
    ...
    window.spTab = function(tabId, event) {
        ...
        if (event && event.currentTarget) {
            ...
            document.querySelectorAll('.sp-tab').forEach(btn => {
                const metin = btn.textContent.trim().toLowerCase();
                if (metin.includes(tabId.toLowerCase()) || (tabId === 'odemeYap' && metin.includes('ödeme yap'))) btn.classList.add('active');
            });
        }
    ...
    // 5. FONKSİYONLAR: MAKBUZ VE GRAFİK
    ...
    window.renderFinancialCharts = function() {
        setTimeout(() => {
            ...
            new window.Chart(ctx, {
                type: 'bar',
                data: { labels: ['Genel Finansal Durum'], datasets: [{ label: 'Gelir', data: [incomeTotal], backgroundColor: '#4ade80' }, { label: 'Gider', data: [expenseTotal], backgroundColor: '#f87171' }] },
...
/* =========================================================================
   DRAGOS AKADEMİ V8 - YENİ ÖZELLİKLER VE MENÜ DÜZELTMESİ (TAM SÜRÜM)
   ========================================================================= */

(function initV8Patch() {
    // 1. KESİN GÖRÜNÜRLÜK CSS'İ (Menüyü zorla ekrana getirir)
    const v8Styles = `
        <style>
            .sp-nav { 
                display: flex !important; 
    ...
    // 2. CHART.JS VE MAKBUZ DOM EKLEMESİ
    if (!window.Chart) {
        const script = document.createElement('script');
        script.src = "https://cdn.jsdelivr.net/npm/chart.js";
        document.head.appendChild(script);
    ...
    // 3. DB MAPPERS VE SP-TAB (MENÜ) DÜZELTMESİ
    setTimeout(() => {
        if (window.DB && window.DB.mappers) {
            if (window.DB.mappers.payments) Object.assign(window.DB.mappers.payments, { category: 'category', accountType: 'account_type' });
            if (window.DB.mappers.settings) Object.assign(window.DB.mappers.settings, { waApiToken: 'wa_api_token', waPhoneId: 'wa_phone_id', waNotifyDay: 'wa_notify_day' });
    ...
    // 4. FONKSİYONLAR: MAKBUZ VE GRAFİK
    window.printReceipt = function(paymentId) {
        if (!window.AppState || !window.AppState.payments) return alert("Veriler yüklenemedi.");
        const payment = window.AppState.payments.find(p => p.id === paymentId);
        if(!payment) return alert("Kayıt bulunamadı.");
