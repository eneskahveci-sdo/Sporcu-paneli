...
async function loadBranchData() {
    ...
    try {
        ...
        // Ön kayıtları yükle (yönetici ise)
...
function updateBranchUI() {
    ...
    // Login ekranındaki okul adını güncelle
    const loginSchoolName = document.getElementById('login-school-name');
    if (loginSchoolName) {
        loginSchoolName.textContent = settings?.schoolName || 'Dragos Futbol Akademisi';
    }
    
    const logoUrl = settings?.logoUrl;
    
    // Sidebar logo
    const sideLogoIcon = document.getElementById('side-logo-icon');
    const sideLogoImg = document.getElementById('side-logo-img');
    if (logoUrl && logoUrl !== DEFAULT_LOGO && logoUrl.startsWith('http')) {
        if (sideLogoIcon) sideLogoIcon.style.display = 'none';
        if (sideLogoImg) { sideLogoImg.src = logoUrl; sideLogoImg.style.display = 'block'; }
    } else {
        if (sideLogoIcon) sideLogoIcon.style.display = '';
        if (sideLogoImg) sideLogoImg.style.display = 'none';
    }
    ...
    if (barAva) {
        if (logoUrl && logoUrl !== DEFAULT_LOGO) {
            ...
            const initials = AppState.currentUser?.name ? AppState.currentUser.name.charAt(0).toUpperCase() : 'A';
            barAva.textContent = initials;
...
function pgSettings() {
    ...
    <div class="card mb3" style="border-left: 4px solid var(--purple)">
        ...
        <div class="flex fca gap3 mb3 fwrap">
            <div id="logo-preview" style="width:80px;height:80px;border-radius:12px;border:2px solid var(--border);background:var(--bg3);display:flex;align-items:center;justify-content:center;font-size:32px;overflow:hidden;flex-shrink:0">
                ${s?.logoUrl && s.logoUrl !== DEFAULT_LOGO ? `<img src="${FormatUtils.escape(s.logoUrl)}" style="width:100%;height:100%;object-fit:cover"/>` : '&#x26BD;'}
            </div>
            <div style="flex:1;min-width:200px">
                <div class="fgr mb2">
                    <label>Logo URL (Harici link)</label>
                    <input id="s-logo-url" type="url" placeholder="https://..." value="${FormatUtils.escape(s?.logoUrl || '')}"/>
                </div>
                <div style="text-align:center;color:var(--text3);font-size:12px;margin:8px 0">— veya —</div>
                <div class="excel-drop-zone" onclick="document.getElementById('logo-file-input').click()">
                    <input type="file" id="logo-file-input" accept="image/*" onchange="handleLogoUpload(this)"/>
                    <div>&#x1F4F7; Dosya Seç / Sürükle Bırak</div>
                    <div style="font-size:12px;color:var(--text3);margin-top:4px">PNG, JPG, SVG desteklenir</div>
                </div>
            </div>
        </div>
        <button class="btn bp" onclick="saveLogoSettings()">Logo Kaydet</button>
        ${s?.logoUrl ? `<button class="btn bd" style="margin-left:8px" onclick="removeLogo()">Logo Kaldır</button>` : ''}
    </div>
...
window.saveGeneralSettings = async function() {
    const obj = {
        ...
        logoUrl: AppState.data.settings?.logoUrl || '',
        netgsmUser: AppState.data.settings?.netgsmUser || '',
        netgsmPass: AppState.data.settings?.netgsmPass || '',
        netgsmHeader: AppState.data.settings?.netgsmHeader || ''
    };
    
    const result = await DB.upsert('settings', DB.mappers.fromSettings(obj));
    if (result) {
        AppState.data.settings = { ...AppState.data.settings, ...obj };
        toast(i18n[AppState.lang].saveSuccess, 'g');
        updateBranchUI();
    }
};
...
window.saveLogoSettings = async function() {
    const logoUrl = UIUtils.getValue('s-logo-url');
    const obj = {
        ...AppState.data.settings,
        logoUrl: logoUrl || ''
    };
...
window.showAddAdminModal = function() {
    modal('Yönetici Ekle', `
    <div class="al al-b mb3" style="font-size:13px">&#x2139;&#xFE0F; Yeni yönetici eklenirken mevcut oturumunuz korunacaktır.</div>
    <div class="fgr mb2">
    ...
    `, [
        ...
        { lbl: 'Kaydet', cls: 'bp', fn: async () => {
            ...
            try {
                ...
                const savedOrgId = AppState.currentOrgId;
                const savedBranchId = AppState.currentBranchId;
                
                // Admin API kullanılamıyorsa, doğrudan users tablosuna kayıt et
                // ve email gönderilmesi için signUp kullan ama session'ı hemen geri yükle
                ...
                try {
                    const { data, error } = await sb.auth.admin?.createUser({
                        ...
                        password: pass,
                        email_confirm: true,
                        user_metadata: { full_name: name, org_id: savedOrgId, branch_id: savedBranchId, role: 'admin' }
                    });
                    ...
                    try {
                        const { data, error } = await sb.auth.signUp({
                            email, password: pass,
                            options: { data: { full_name: name, org_id: savedOrgId, branch_id: savedBranchId, role: 'admin' } }
                        });
                        if (!error && data?.user) newUserId = data.user.id;
                    } catch(e2) { /* ignore */ }
                    ...
                    // Oturumu geri yükle
                    try {
                        await sb.auth.signInWithPassword({ email: savedUser.email || '', password: '' });
                    } catch(e3) { /* ignore */ }
                }
                
                // Users tablosuna kayıt et
                ...
                // AppState'i ve storage'ı geri yükle
                AppState.currentUser = savedUser;
                AppState.currentOrgId = savedOrgId;
                AppState.currentBranchId = savedBranchId;
                StorageManager.set('sporcu_app_user', savedUser);
                StorageManager.set('sporcu_app_org', savedOrgId);
                StorageManager.set('sporcu_app_branch', savedBranchId);
                
                toast('Yönetici kaydı oluşturuldu! E-posta doğrulaması gönderildi.', 'g');
                closeModal();
            } catch (e) {
