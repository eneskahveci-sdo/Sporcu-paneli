...
var _sb = null;
function getSB() {
  if (!_sb) { try { _sb = supabase.createClient(SUPA_URL, SUPA_KEY); } catch(e) { console.error('Supabase init failed:', e); } }
  return _sb;
}
...
async function restoreSession() {
  showLoading();
  try {
    var sb = getSB(); var { data } = await sb.auth.getSession();
...
async function supaPost(t, d) {
  try { var db = getSB(); if (!db) return null; var { data, error } = await db.from(t).insert(d).select(); if (error) { console.error('HATA:', error.message); return null; } return data; } catch(e) { return null; }
}
...
async function supaUpsert(t, d) {
  try {
    ...
    if (error) { console.error('Veritabanı Kayıt Hatası (' + t + '):', error.message); return null; }
    return data;
  } catch(e) { console.error('Sistem Hatası:', e); return null; }
}

async function supaDelete(t, f) {
  try { var db = getSB(); if (!db) return false; var q = db.from(t).delete(); Object.keys(f).forEach(function(k) { q = q.eq(k, f[k]); }); var { error } = await q; if (error) return false; return true; } catch(e) { return false; }
}

async function supaPatch(t, d, f) {
  try { var db = getSB(); if (!db) return null; var q = db.from(t).update(d); Object.keys(f).forEach(function(k) { q = q.eq(k, f[k]); }); var { data, error } = await q.select(); if (error) return null; return data; } catch(e) { return null; }
}
...
function pgSettings() { 
  return '<div class="ph"><div class="stit">Ayarlar</div><div class="ssub">Sistem ayarları ve yönetim</div></div>' +
    ...
    '</div>' +
    '<div class="card mb3"><div class="tw6 tsm mb2">Kurum Bilgileri</div>' + row('Kurum Adı', settings.schoolName || '-') + row('Adres', settings.address || '-') + row('Telefon', settings.ownerPhone || '-') + '</div>' +
...
async function handleLogoUpload(input) {
  ...
  reader.onload = function(e) {
    ...
    img.onload = async function() {
      ...
      await saveS(); updateBranchUI(); toast('Logo başarıyla güncellendi!', 'g'); hideLoading(); go('settings');
    };
