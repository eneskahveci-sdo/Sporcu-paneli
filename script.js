    ...
    mappers: {
        ...
        toPayment(r) {
            return {
                ...
                source: r.source || 'manual',      // 'manual'|'bank_transfer'|'cash'|'credit_card'|'paytr'
                notifStatus: r.notif_status || '',  // 'pending_approval'|'approved'|''
                payMethod: r.pay_method || ''       // 'nakit'|'kredi_karti'|'havale'|'paytr'
            };
        },
        fromPayment(p) {
            return {
...
async function restoreSession() {
    ...
    try {
        ...
        if (storedSporcu) {
            ...
            // Logo ve okul adını tüm ekranlara uygula
            applyLogoEverywhere(AppState.data.settings?.logoUrl || '');
        ...
        if (storedUser) {
            ...
            // Admin ise Supabase session kontrolü yap
            if (storedUser.role === 'admin') {
                ...
                if (sb) {
                    try {
                        ...
                        if (!session) {
                            // Session yok ama localStorage var — temizle
                            ...
                            return; // Login ekranı gösterilsin
                        }
                    } catch(e) {
                        console.warn('Session check failed:', e);
                        // Sessiz devam et — offline olabilir
                    }
                }
            }
        ...
        // Sadece admin session hatasında temizle, sporcu verilerini tutabilirsin
        StorageManager.remove('sporcu_app_user');
        StorageManager.remove('sporcu_app_org');
        StorageManager.remove('sporcu_app_branch');
    } finally {
...
async function loadBranchData() {
    ...
    try {
        ...
        // Logoyu hemen tüm ekranlara uygula (giriş ekranı dahil)
        applyLogoEverywhere(AppState.data.settings?.logoUrl || '');
        ...
        // Ön kayıtları yükle (yönetici ve antrenör görebilir)
...
function applyLogoEverywhere(logoUrl) {
    ...
    // 1) GİRİŞ EKRANI LOGO (#login-logo)
    const loginLogo = document.getElementById('login-logo');
    if (loginLogo) {
        if (hasLogo) {
            loginLogo.textContent = '';
            loginLogo.style.backgroundImage = 'url("' + logoUrl + '")';
    ...
    if (spHead) {
        ...
        if (hasLogo) {
            if (!spLogoEl) {
                ...
                spLogoEl.alt = 'Logo';
                spLogoEl.style.cssText = 'width:36px;height:36px;border-radius:8px;object-fit:cover;border:1px solid var(--border);flex-shrink:0';
...
function pgAthletes() {
    ...
    // Ön kayıt bölümü — yönetici ve antrenör görebilir, yönetici kayda dönüştürebilir
    const onKayitSection = onKayitlar.length > 0 ? `
    <div class="card mb3" style="border-left:4px solid var(--yellow)">
        <div class="flex fjb fca mb3">
            <div class="tw6 tsm">📝 Ön Kayıt Başvuruları <span style="background:var(--yellow);color:#000;border-radius:10px;padding:1px 8px;font-size:11px;font-weight:800;margin-left:6px">${pendingOnKayit.length} Yeni</span></div>
            <button class="btn btn-sm bs" onclick="go('athletes')">&#x21BB; Yenile</button>
        </div>
        <div class="tw" style="overflow-x:auto">
            <table>
                ...
                <tbody>
                ${onKayitlar.map(ok => `
                <tr style="${ok.status === 'new' ? 'background:rgba(234,179,8,.07)' : 'opacity:.6'}">
                    <td class="ts">${ok.createdAt ? DateUtils.format(ok.createdAt) : '-'}</td>
                    <td class="tw6">${FormatUtils.escape(ok.fn + ' ' + ok.ln)}</td>
                    <td class="ts">${FormatUtils.escape(ok.tc || '-')}</td>
                    <td class="ts">${ok.bd ? DateUtils.format(ok.bd) : '-'}</td>
                    <td>${FormatUtils.escape(ok.className || '-')}</td>
                    <td class="ts">${FormatUtils.escape(ok.parentName || '-')}<br><small>${FormatUtils.escape(ok.parentPhone || '')}</small></td>
                    ...
                    ${isAdmin ? `<td>
                        ...
                        <button class="btn btn-xs bd" onclick="delOnKayit('${ok.id}')" style="margin-left:4px">Sil</button>
                    </td>` : ''}
                </tr>`).join('')}
                </tbody>
            </table>
...
function pgPayments() {
    ...
    <div class="card mb3" style="border-left:4px solid var(--yellow)">
        ...
        ${pendingNotifs.map(p => {
            const ath = AppState.data.athletes.find(a => a.id === p.aid);
...
window.approvePayment = async function(id) {
    ...
    if (!p) return;
    const obj = { ...p, st: 'completed', notifStatus: 'approved' };
    const result = await DB.upsert('payments', DB.mappers.fromPayment(obj));
    if (result) {
        const idx = AppState.data.payments.findIndex(x => x.id === id);
        if (idx >= 0) AppState.data.payments[idx] = obj;
        toast(`✅ ${p.an} ödemesi onaylandı ve kaydedildi!`, 'g');
        go('payments');
    }
};
...
window.saveLogoSettings = async function() {
    ...
    if (result) {
        ...
        // Ayarlar sayfasını yenile (kaldır butonu görünsün)
        go('settings');
    }
};
...
function spOdemeYap() {
    ...
    <div class="card mb3">
        <div class="profile-header" style="margin-bottom:16px">
            <div class="profile-header-content">
                <div class="profile-avatar" style="width:60px;height:60px;font-size:24px">${FormatUtils.initials(a.fn, a.ln)}</div>
                <div class="profile-info">
                    <div class="profile-name" style="font-size:18px">${FormatUtils.escape(`${a.fn} ${a.ln}`)}</div>
                    <div class="profile-meta">
        ...
        <div class="fgr mb2">
            ...
            <input id="sp-amt" type="number" inputmode="decimal" value="${a.fee || ''}"/>
        </div>
        <div class="fgr mb3">
            <label>Açıklama</label>
            <input id="sp-desc" value="${DateUtils.format(DateUtils.today())} tarihli ödeme"/>
        </div>
        
        <div class="tw6 ts mb2">Ödeme Yöntemi Seçin:</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
            <button class="pay-method-btn" id="pm-nakit" onclick="selectPayMethod('nakit')" style="padding:14px 10px;border:2px solid var(--border);border-radius:12px;background:var(--bg3);cursor:pointer;transition:.2s">
                <div style="font-size:24px;margin-bottom:4px">💵</div>
                <div class="tw6 ts">Nakit</div>
            </button>
            <button class="pay-method-btn" id="pm-kredi_karti" onclick="selectPayMethod('kredi_karti')" style="padding:14px 10px;border:2px solid var(--border);border-radius:12px;background:var(--bg3);cursor:pointer;transition:.2s">
                <div style="font-size:24px;margin-bottom:4px">💳</div>
                <div class="tw6 ts">Kredi Kartı</div>
            </button>
            ${hasBank ? `
            <button class="pay-method-btn" id="pm-havale" onclick="selectPayMethod('havale')" style="padding:14px 10px;border:2px solid var(--border);border-radius:12px;background:var(--bg3);cursor:pointer;transition:.2s">
                <div style="font-size:24px;margin-bottom:4px">🏦</div>
                <div class="tw6 ts">Havale / EFT</div>
            </button>` : ''}
            ${hasPayTR ? `
            <button class="pay-method-btn" id="pm-paytr" onclick="selectPayMethod('paytr')" style="padding:14px 10px;border:2px solid var(--border);border-radius:12px;background:var(--bg3);cursor:pointer;transition:.2s">
                <div style="font-size:24px;margin-bottom:4px">🔵</div>
                <div class="tw6 ts">PayTR (Online)</div>
            </button>` : ''}
        </div>
        
        <div id="pay-method-detail"></div>
        
        <button class="btn bp w100 mt2" id="pay-submit-btn" style="display:none" onclick="submitSpPayment()">
            Ödeme / Bildirim Gönder
        </button>
    </div>`;
}
...
window.selectPayMethod = function(method) {
    ...
    if (activeBtn) {
        activeBtn.style.borderColor = 'var(--blue2)';
        activeBtn.style.background = 'rgba(59,130,246,.1)';
    }
    ...
    if (method === 'nakit') {
        detail.innerHTML = `
        <div class="al al-b" style="border-radius:10px;padding:14px">
            <div class="tw6 mb1">💵 Nakit Ödeme</div>
            <p class="ts tm">Nakit ödeme yapmak için akademi merkezine gelin. Ödemenizi teslim ettikten sonra bu ekrandan bildirim yapın, yönetici onaylayacaktır.</p>
        </div>`;
        ...
        detail.innerHTML = `
        <div class="al al-b" style="border-radius:10px;padding:14px">
            <div class="tw6 mb1">💳 Kredi Kartı</div>
            <p class="ts tm">Kredi kartıyla ödeme yapmak için akademi merkezine gelin veya yöneticiye başvurun. Ödemenizi yaptıktan sonra bildirim gönderin.</p>
        </div>`;
        submitBtn.textContent = '📩 Kredi Kartı Ödeme Bildirimi Gönder';
        submitBtn.style.display = 'block';
    } else if (method === 'havale') {
        detail.innerHTML = `
        <div class="al al-b" style="border-radius:10px;padding:14px;margin-bottom:10px">
            <div class="tw6 mb2">🏦 Havale / EFT Bilgileri</div>
            <div class="ts"><strong>Banka:</strong> ${FormatUtils.escape(s?.bankName || '-')}</div>
            <div class="ts"><strong>Alıcı:</strong> ${FormatUtils.escape(s?.accountName || '-')}</div>
            <div class="ts" style="word-break:break-all"><strong>IBAN:</strong> ${FormatUtils.escape(s?.iban || '-')}</div>
        ...
        detail.innerHTML = `<div class="al al-b" style="border-radius:10px;padding:14px"><div class="tw6 mb1">🔵 PayTR ile Online Ödeme</div><p class="ts tm">Güvenli ödeme sayfasına yönlendirileceksiniz.</p></div>`;
...
window.submitSpPayment = async function() {
    ...
    if (!amt || amt <= 0) { toast('Lütfen geçerli bir tutar giriniz!', 'e'); return; }
    ...
    // Nakit, kredi kartı veya havale bildirimi — DB'ye "onay bekliyor" olarak kaydet
    const payObj = {
        id: generateId(),
        aid: a.id,
        an: `${a.fn} ${a.ln}`,
        amt: amt,
        ds: desc || 'Veli bildirimi',
        st: 'pending',
        dt: DateUtils.today(),
        ty: 'income',
        serviceName: desc || 'Veli bildirimi',
    ...
    if (sb) {
        try {
            ...
            toast(`✅ ${methodLabel} ödeme bildiriminiz alındı! Yönetici onayından sonra işleme alınacaktır.`, 'g');
            spTab('odemeler');
        } catch(e) {
            toast('Bildirim gönderilemedi: ' + (e.message || e), 'e');
        }
    } else {
        toast('Bağlantı hatası. Lütfen tekrar deneyin.', 'e');
    }
};
...
async function loadOnKayitlar() {
    try {
        const sb = getSupabase();
        if (!sb || !AppState.currentBranchId) return;
        const { data } = await sb.from('on_kayitlar').select('*').eq('branch_id', AppState.currentBranchId).order('created_at', { ascending: false });
        if (data) {
            AppState.data.onKayitlar = data.map(r => ({
                id: r.id, studentName: r.student_name, fn: r.fn, ln: r.ln, bd: r.bd, tc: r.tc,
                clsId: r.cls_id, className: r.class_name, parentName: r.parent_name,
                parentPhone: r.parent_phone, status: r.status, createdAt: r.created_at
            }));
