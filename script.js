...
function applyLogoEverywhere(logoUrl) {
    ...
    if (loginLogo) {
        if (hasLogo) {
            loginLogo.innerHTML = `<img src="${logoUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:14px;display:block;" onerror="this.parentElement.innerHTML='&#x26BD;'"/>`;
...
function spOdemeYap() {
    ...
    // Bu sporcuya ait ödeme planları (pending/overdue)
    const myPlans = (AppState.data.payments || [])
        .filter(p => p.aid === a.id && p.source === 'plan' && p.st !== 'completed')
        .sort((x, y) => x.dt.localeCompare(y.dt));

    const planRows = myPlans.length > 0 ? myPlans.map(p => `
    <div style="padding:14px;background:var(--bg3);border-radius:12px;border:1px solid ${p.st==='overdue'?'var(--red)':'var(--border)'};margin-bottom:10px">
        <div class="flex fjb fca fwrap gap2">
            <div>
                <div class="tw6">${FormatUtils.escape(p.ds || p.serviceName || 'Aidat')}</div>
                <div class="ts tm">Tutar: <strong class="tg">${FormatUtils.currency(p.amt)}</strong>
                    ${p.st==='overdue'?'<span class="bg bg-r" style="margin-left:6px">Gecikmiş</span>':''}
                </div>
            </div>
            <button class="btn bp btn-sm" onclick="spPayPlan('${p.id}')">💳 Öde</button>
        </div>
    </div>`) : `<div style="text-align:center;padding:30px;color:var(--text3)">
        <div style="font-size:36px;margin-bottom:8px">✅</div>
        <div class="tw6">Bekleyen ödeme planı yok.</div>
    </div>`;
    ...
    <div class="card mb3">
        <div class="profile-header" style="margin-bottom:12px">
            <div class="profile-header-content">
                <div class="profile-avatar" style="width:52px;height:52px;font-size:20px">${FormatUtils.initials(a.fn, a.ln)}</div>
                <div class="profile-info">
                    <div class="profile-name" style="font-size:17px">${FormatUtils.escape(`${a.fn} ${a.ln}`)}</div>
                    <div class="ts tm">Aylık Ücret: <strong>${FormatUtils.currency(a.fee)}</strong></div>
                </div>
            </div>
        </div>
        <div class="tw6 ts mb2">📋 Bekleyen Ödemelerim</div>
        ${planRows}
    </div>

    <div class="card" id="sp-pay-form" style="display:none">
        <div class="tw6 tsm mb3">💳 Ödeme Yöntemi Seç</div>
        <div id="sp-plan-info" style="padding:10px;background:var(--bg3);border-radius:8px;margin-bottom:14px;font-size:14px"></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">
            <button class="pay-method-btn" id="pm-nakit" onclick="selectPayMethod('nakit')" style="padding:14px 10px;border:2px solid var(--border);border-radius:12px;background:var(--bg3);cursor:pointer">
                <div style="font-size:22px;margin-bottom:4px">💵</div><div class="tw6 ts">Nakit</div>
            </button>
            <button class="pay-method-btn" id="pm-kredi_karti" onclick="selectPayMethod('kredi_karti')" style="padding:14px 10px;border:2px solid var(--border);border-radius:12px;background:var(--bg3);cursor:pointer">
                <div style="font-size:22px;margin-bottom:4px">💳</div><div class="tw6 ts">Kredi Kartı</div>
            </button>
            ${hasBank ? `<button class="pay-method-btn" id="pm-havale" onclick="selectPayMethod('havale')" style="padding:14px 10px;border:2px solid var(--border);border-radius:12px;background:var(--bg3);cursor:pointer">
                <div style="font-size:22px;margin-bottom:4px">🏦</div><div class="tw6 ts">Havale/EFT</div>
            </button>` : ''}
            ${hasPayTR ? `<button class="pay-method-btn" id="pm-paytr" onclick="selectPayMethod('paytr')" style="padding:14px 10px;border:2px solid var(--border);border-radius:12px;background:var(--bg3);cursor:pointer">
                <div style="font-size:22px;margin-bottom:4px">🔵</div><div class="tw6 ts">PayTR Online</div>
            </button>` : ''}
        </div>
        <div id="pay-method-detail"></div>
        <div class="fgr mb2">
            <label>Açıklama</label>
            <input id="sp-desc" value=""/>
        </div>
        <button class="btn bp w100 mt2" id="pay-submit-btn" style="display:none" onclick="submitSpPayment()">Bildirim Gönder</button>
        <button class="btn bs w100 mt2" onclick="document.getElementById('sp-pay-form').style.display='none'">← Geri</button>
    </div>`;
}
