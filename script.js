...
async function loadLogoForLoginScreen() {
    try {
        const sb = getSupabase();
        if (!sb) return;
        const { data } = await sb.from('settings').select('logo_url, school_name').limit(1).single();
        if (data) {
            if (data.logo_url) applyLogoEverywhere(data.logo_url);
            const loginSchoolName = document.getElementById('login-school-name');
            if (loginSchoolName && data.school_name) loginSchoolName.textContent = data.school_name;
        }
    } catch(e) {
...
function applyLogoEverywhere(logoUrl) {
    ...
    if (loginLogo) {
        if (hasLogo) {
            ...
            loginLogo.innerHTML = '';
            // style attribute olarak yaz — CSS shorthand'i override eder
            loginLogo.setAttribute('style',
