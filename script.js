...
function pgSettings() {
    ...
    <div class="card mb3" style="border-left: 4px solid #0070f3">
        ...
        <div class="al al-b mb3" style="font-size:12px">
            ...
            Bilgilerinizi PayTR merchant panelinizden alın.<br>
        ...
        <div class="fgr mb2">
            ...
            <input id="s-paytr-mid" type="text" placeholder="PayTR Merchant ID" value="${FormatUtils.escape(s?.paytrMerchantId || '')}"/>
        </div>
        <div class="fgr mb2">
            <label>Merchant Key</label>
            <input id="s-paytr-key" type="password" placeholder="PayTR Merchant Key" value="${FormatUtils.escape(s?.paytrMerchantKey || '')}"/>
        </div>
        <div class="fgr mb2">
            <label>Merchant Salt</label>
            <input id="s-paytr-salt" type="password" placeholder="PayTR Merchant Salt" value="${FormatUtils.escape(s?.paytrMerchantSalt || '')}"/>
        </div>
...
window.savePayTRSettings = async function() {
    ...
    const mkey = document.getElementById('s-paytr-key')?.value.trim() || '';
    const msal = document.getElementById('s-paytr-salt')?.value.trim() || '';
    const active = document.getElementById('s-paytr-active')?.checked || false;
    if (active && (!mid || !mkey || !msal)) {
        toast('PayTR aktif edilmek için Merchant ID, Key ve Salt zorunludur!', 'e');
        return;
    }
    const obj = {
        ...(AppState.data.settings || {}),
        paytrActive: active,
        paytrMerchantId: mid,
        paytrMerchantKey: mkey,
        paytrMerchantSalt: msal
    };
    const result = await DB.upsert('settings', DB.mappers.fromSettings(obj));
    if (result) {
        AppState.data.settings = obj;
